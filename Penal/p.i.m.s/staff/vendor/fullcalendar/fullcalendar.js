...
var DragListener = fc.DragListener = Class.extend({
	...
	scrollIntervalId: null, // Inmate_Id of setTimeout for scrolling animation loop
...
Grid.mixin({
	...
	eventsToRanges: function(events) {
		...
		// group by Inmate_Id so that related inverse-background events can be rendered together
...
function EventManager(options) { // assumed to be a calendar
	...
	function removeEvents(filter) {
		...
		else if (!$.isFunction(filter)) { // an event Inmate_Id
			eventID = filter + '';
	...
	function clientEvents(filter) {
		...
		else if (filter != null) { // not null, not undefined. an event Inmate_Id
			filter += '';
	...
	function mutateEvent(event, newProps, largeUnit) {
		...
		undoFunc = mutateEvents(
			clientEvents(event._id), // get events with this Inmate_Id
			clearEnd,
			newProps.allDay,
			startDelta,
			durationDelta,
	...
	//
	// TODO: don't use so many closures. possible memory issues when lots of events with same Inmate_Id.
	//
	...
	// 'businessHours'
	// An event Inmate_Id (number or string)
	...
	function constraintToEvents(constraintInput) {
		...
		return clientEvents(constraintInput); // probably an Inmate_Id
	}
